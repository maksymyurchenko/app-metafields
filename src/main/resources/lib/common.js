...
exports.getTheConfig = function (site) {
  ...
  if (app.config && !config.disableAppConfig) {
    for (var prop in app.config) {
      ...
      if (prop !== "config.filename" && prop !== "service.pid") {
        // Default props for .cfg-files, not to use further.
...
exports.getSite = function (siteUrl) {
  ...
  var sitesResult = libs.content.query({
    query:
      "_path LIKE '/content/*' AND _name LIKE '" +
      siteUrl +
      "' AND data.siteConfig.applicationKey = '" +
      app.name +
      "'",
  ...
  for (var i = 0; i < pathLength; i++) {
    if (paths[i]) {
      ...
      if (value) {
        ...
        else break; // Expect the first property in the string is the most important one to use
      } // if value
    } // if paths[i]
  } // for
  return value;
...
exports.getAppendix = function (site, isFrontpage) {
  ...
  var titleAppendix = "";
  if (
    siteConfig.titleBehaviour ||
    !siteConfig.hasOwnProperty("titleBehaviour")
  ) {
    var separator = siteConfig.titleSeparator || "-";
    var titleRemoveOnFrontpage = siteConfig.hasOwnProperty(
      "titleFrontpageBehaviour"
    )
      ? siteConfig.titleFrontpageBehaviour
...
exports.getPageTitle = function (content, site) {
  ...
  var userDefinedArray = userDefinedPaths
    ? commaStringToArray(userDefinedPaths)
    : [];
  var userDefinedValue = userDefinedPaths
    ? findValueInJson(content, userDefinedArray)
    : null;
  ...
  var metaTitle = setWithMixin
    ? stringOrNull(content.x[appNamePath][mixinPath].seoTitle) // Get from mixin
...
exports.getMetaDescription = function (content, site) {
  ...
  var metaDescription = setWithMixin
    ? content.x[appNamePath][mixinPath].seoDescription // Get from mixin
    : userDefinedValue ||
      content.data.preface ||
      content.data.description ||
      content.data.summary || // Use typical content summary names
      siteConfig.seoDescription || // Use default for site
      getDescriptionFromParts(content) || // Trying getting description from parts
...
function getDescriptionFromParts(content) {
  if (
    content &&
    content.page &&
    ...
    content.page.regions.main.components
  ) {
    var components = libs.util.data.forceArray(
      content.page.regions.main.components
    );
    for (var i = 0; i < components.length; i++) {
      if (components[i].type === "text") {
        return (
          components[i].text.replace(/(<([^>]+)>)/gi, "").substring(0, 250) +
          "..."
        );
      }
    }
...
exports.getOpenGraphImage = function (
  content,
  site,
  defaultImg,
  defaultImgPrescaled
  ...
  var imageArray = libs.util.data.forceArray(
    setWithMixin
      ? stringOrNull(content.x[appNamePath][mixinPath].seoImage)
      : userDefinedValue || content.data.image || content.data.images || []
  );
  ...
  if (imageArray.length || (defaultImg && !defaultImgPrescaled)) {
    ...
    if (theImage) {
      if (theImage.data.media.attachment) {
        mimeType =
          theImage.attachments[theImage.data.media.attachment].mimeType; // Get the actual mimeType
